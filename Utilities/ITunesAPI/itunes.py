...
class _Request(object):
    ...
    def _get_cached_response(self):
        ...
        if not self._is_cached():
            response = self._download_response()
            response_file = open(os.path.join(_get_cache_dir(), self._get_cache_key()), "wb")
            response_file.write(response.encode('utf8'))
            response_file.close()
        return open(os.path.join(_get_cache_dir(), self._get_cache_key()), "rb").read().decode('utf8')
...
class Software(Track):
    ...
    def _set(self, json):
        ...
        self._set_supported_devices(json)
    ...
    def _set_avg_rating(self, json):
            ...
            self.avg_rating_current_version = json.get('averageUserRatingForCurrentVersion', None)

            self.avg_rating = json.get('averageUserRating', None)

    def _set_num_ratings(self, json):
            self.num_ratings_current_version = json.get('userRatingCountForCurrentVersion', None)
    ...
    def get_avg_rating_current_version(self):
        return self.avg_rating_current_version
    def get_num_ratings_current_version(self):
        return self.num_ratings_current_version
